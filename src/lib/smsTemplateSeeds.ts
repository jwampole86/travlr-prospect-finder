...
export const SMS_TEMPLATE_SEEDS: SMSTemplateSeed[] = [
  ...
  // ── 3. Check-In / Re-Engage ────────────────────────────────────────────────
  {
    ...
    tag: 'follow up',
    category: 'Follow-Up',
    body: `Hi {{contactName}}, wanted to check back in regarding {{address}}. Is the property still available? We remain interested, and {{localBlurb}}. Happy to reconnect whenever the timing makes sense.`,
    requiredVars: ['{{senderName}}', '{{contactName}}', '{{address}}', '{{localBlurb}}'],
    blockingVars: ['{{senderName}}', '{{address}}'],
  },
  ...
  // ── 4. Proposal Introduction ───────────────────────────────────────────────
  {
    ...
    tag: 'proposal',
    category: 'Proposal',
    body: `Hi {{contactName}}, thanks again for speaking with me about {{address}}. I'd like to formally propose {{proposedRent}}/month for {{leaseTerm}}, starting {{proposedStartDate}}. TRAVLR would handle the full guest and property experience, and {{localBlurb}}. Happy to talk through any questions.`,
    requiredVars: [
      '{{senderName}}',
      '{{contactName}}',
      '{{address}}',
  ...
  // ── 5. Closing / Contract ──────────────────────────────────────────────────
  {
    ...
    tag: 'closing',
    category: 'Closing',
    body: `Hi {{contactName}}, great news — we're ready to move forward with {{address}}. {{localBlurb}}, and we're excited to bring that same full-service approach to your property. I'll send over the agreement for review, then we'll coordinate the walkthrough and start date.`,
    requiredVars: ['{{senderName}}', '{{contactName}}', '{{address}}', '{{localBlurb}}'],
    blockingVars: ['{{senderName}}', '{{address}}'],
  },
];
